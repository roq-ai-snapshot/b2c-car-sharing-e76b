...
export function CarListPage(props: CarListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getCars({
        relations: ['company', 'booking.count', 'location.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['make.contains', 'model.contains', 'color.contains', 'license_plate.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'license_plate', header: 'License Plate', accessorKey: 'license_plate' },
    ...
    hasAccess('location', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.location,
        }
      : null,
