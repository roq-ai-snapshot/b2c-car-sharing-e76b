            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('dashboard', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/dashboards/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Last Login"
                  text={data?.last_login ? format(parseISO(data?.last_login as unknown as string), 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Active Status" text={data?.active_status} />

                <FormListItem label="Assigned Cars" text={data?.assigned_cars} />

                <FormListItem label="Total Bookings" text={data?.total_bookings} />
