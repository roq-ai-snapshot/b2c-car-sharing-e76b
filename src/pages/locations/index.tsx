...
import { getLocations, deleteLocationById } from 'apiSdk/locations';
import { LocationInterface } from 'interfaces/location';

type ColumnType = ColumnDef<LocationInterface, unknown>;
...
export function LocationListPage(props: LocationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getLocations({
        ...
        order: params.order,
        searchTermKeys: ['address.contains', 'city.contains', 'state.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LocationInterface>>(
    () => `/locations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: LocationInterface) => {
    if (hasAccess('location', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/locations/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'latitude', header: 'Latitude', accessorKey: 'latitude' },
    { id: 'longitude', header: 'Longitude', accessorKey: 'longitude' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/locations/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('location', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/locations/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="location" />
        </Flex>

        {hasAccess('location', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/locations/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
